    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $validatedData = $request->validate([
                ...
                'status' => 'required|in:active,on_leave,on_permission',
                // Validation des personnes à prévenir - optionnel
                'personnes' => 'nullable|array',
                'personnes.*.nom' => 'nullable|required_with:personnes.*.prenom|string|max:255',
                'personnes.*.prenom' => 'nullable|required_with:personnes.*.nom|string|max:255',
                'personnes.*.contact' => 'nullable|required_with:personnes.*.nom|string|max:255',
                'personnes.*.profession' => 'nullable|string|max:255',
                'personnes.*.adresse' => 'nullable|string|max:255',
            ]);

            // Convertir les valeurs du formulaire à celles acceptées par la base de données
            $validatedData['sexe'] = strtolower(str_replace('Féminin', 'féminin', str_replace('Masculin', 'masculin', $request->input('sexe'))));

            // Mappage du status
            $statusMap = [
                'actif' => 'active',
                'inactif' => 'on_leave',
                'en attente' => 'on_permission'
            ];
            $validatedData['status'] = $statusMap[$request->input('status')] ?? 'active';
            ...
            // Enregistrer les personnes à prévenir (seulement si remplies)
            $personnes = $request->input('personnes', []);
            foreach ($personnes as $personne) {
                // Sauter si les champs obligatoires sont vides
                if (empty($personne['nom']) && empty($personne['prenom']) && empty($personne['contact'])) {
                    continue;
                }
